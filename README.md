...
> and the declared fixture corpora (191 programs across root/3D/classic/synth/mixer/points/filter/v104/tiled gates) render
> with zero failures — 160 strict byte-level passes plus 31 measured, bounded exceptions.
...
live-DSL compiler vs the reference oracle) and **191/191 rendered fixtures graded with
zero failures** — 160 strict `PASS` and 31 narrowly bounded `ALLOWED_NEAR` (root corpus 5,
3D corpus 3, classicNoisedeck corpus 2, synth corpus 3, mixer corpus 2, points corpus 8, filter corpus 4 (batch 1 of 3), v104 corpus 4), every one pinned
...
before comparison); the `filter` namespace is 64/113 pixel-verified and continuing in
