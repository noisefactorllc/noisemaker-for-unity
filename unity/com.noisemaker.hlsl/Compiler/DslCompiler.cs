    ...
    public static class DslCompiler
    {
        ...
        private static OrderedMap<string, TextureSpec> BuildTextures(ExpandResult expanded)
        {
            ...
            foreach (var kv in expanded.TextureSpecs)
            {
                ...
                // `filter` is 3D-only (whitelist "nearest"/"linear", matching the
                // Expander/GraphLoader and upstream effect-validator.js);
                // `mipmaps`/`persistent` are 2D-only — same branch structure as
                // compiler.js extractTextureSpecs().
                if (spec.Is3D)
                {
                    if (src.Filter == "nearest" || src.Filter == "linear") spec.Filter = src.Filter;
                }
                else { spec.Mipmaps = src.Mipmaps; spec.Persistent = src.Persistent; }
        ...
        private static void WriteTextureSpec(StringBuilder sb, TextureSpec t)
        {
            ...
            if (t.Is3D)
            {
                if (!string.IsNullOrEmpty(t.Filter)) { sb.Append(','); WriteKey(sb, "filter"); WriteJsonString(sb, t.Filter); }
            }
            else
            {
                if (t.Mipmaps.HasValue) { sb.Append(','); WriteKey(sb, "mipmaps"); sb.Append(t.Mipmaps.Value ? "true" : "false"); }
                if (t.Persistent.HasValue) { sb.Append(','); WriteKey(sb, "persistent"); sb.Append(t.Persistent.Value ? "true" : "false"); }
            }
            sb.Append('}');
        }
