#!/usr/bin/env bash
# End-to-end 30-fixture root parity gate: reference goldens, Unity candidates,
# then fail-closed grading with the repository's measured exception policy.
# Covers every root corpus fixture NOT in 3d-manifest.tsv (the 3D fixtures are
# graded by 3d-verify.sh with the 3D tolerance/exception policy).
set -euo pipefail
...
MANIFEST="$HERE/programs/manifest.tsv"
EXCEPTIONS="$HERE/programs/exceptions.json"
NM_ROOT_TMP="$(mktemp -d "${TMPDIR:-/tmp}/nm-unity-root.XXXXXX")"
trap 'rm -rf -- "$NM_ROOT_TMP"' EXIT
GOLD="$NM_ROOT_TMP/gold"
CAND="$NM_ROOT_TMP/candidate"
RENDER_MANIFEST="$NM_ROOT_TMP/render.tsv"
UNITY_LOG="$NM_ROOT_TMP/unity.log"
mkdir -p "$GOLD" "$CAND"
...
if ! grep -Fq "[NMParity] dsl batch done: $EXPECTED_COUNT ok, 0 fail" "$UNITY_LOG"; then
  tail -80 "$UNITY_LOG" >&2
  echo "Unity root batch did not render the exact manifest successfully" >&2
  exit 1
fi

"$PYTHON" "$HERE/batch-compare.py" "$GOLD" "$CAND" \
  --out "$NM_ROOT_TMP/report.json" \
