#!/usr/bin/env bash
# End-to-end 12-fixture mixer parity gate: reference goldens, Unity candidates,
# then fail-closed grading with the repository's measured exception policy.
# Covers the 12 mixer effects not in the root corpus (alphaMask, blendMode,
# and mashup are in root-verify.sh). Together, all 15 mixer effects are verified
# for rendered pixel parity.
set -euo pipefail
...
MANIFEST="$HERE/programs/mixer-manifest.tsv"
EXCEPTIONS="$HERE/programs/mixer-exceptions.json"
NM_MIXER_TMP="$(mktemp -d "${TMPDIR:-/tmp}/nm-unity-mixer.XXXXXX")"
trap 'rm -rf -- "$NM_MIXER_TMP"' EXIT
GOLD="$NM_MIXER_TMP/gold"
CAND="$NM_MIXER_TMP/candidate"
RENDER_MANIFEST="$NM_MIXER_TMP/render.tsv"
UNITY_LOG="$NM_MIXER_TMP/unity.log"
mkdir -p "$GOLD" "$CAND"
...
if ! grep -Fq "[NMParity] dsl batch done: $EXPECTED_COUNT ok, 0 fail" "$UNITY_LOG"; then
  tail -80 "$UNITY_LOG" >&2
  echo "Unity mixer batch did not render the exact manifest successfully" >&2
  exit 1
fi

"$PYTHON" "$HERE/batch-compare.py" "$GOLD" "$CAND" \
  --out "$NM_MIXER_TMP/report.json" \
