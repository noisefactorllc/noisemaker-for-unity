...
- Observed behavior: Package metadata declares Unity 6 (`6000.0`), aligning with verified `6000.3.16f1`. The obsolete 2021.3 minimum declaration was dropped across package manifest and docs. Pass 2 (2026-09-24): current graph and image gates executed at the pinned authority — graph 316/316, render 112/112 (100 PASS + 12 measured bounded exceptions, exit 0), compiler contract tests PASS. Pass 7 (2026-09-24): `classic-verify.sh` added 17 new fixtures covering all remaining `classicNoisedeck/*` effects (15 PASS, 2 ALLOWED_NEAR, exit 0). The entire `classicNoisedeck` namespace (20/20 effects) is now pixel-parity qualified. Full suite expanded to 129 fixtures (115 PASS, 14 bounded, exit 0). Per-effect coverage reached 91/210 IDs. What remains open is the broader host/platform matrix (Windows/Linux) and remaining per-effect parameter/state/input breadth.
