...
These gates establish rendered parity for the declared fixture corpora at the pinned authority. Per-effect parameter/state/input breadth remains open under GAP-001; the installed-workflow qualification (GAP-002) closed on 2026-09-26 (pass 17); distribution (GAP-003) qualification remains open.
...
Recorded explicitly (GAP-002 closed, 2026-09-26 pass 17): Linux and Windows are unavailable to this automation — the Linux editor (`6000.3.16f1`, `a56f230f6470`, unattended official tarball on x86_64, kernel 6.8.0-134) exits batch mode **198** "No valid Unity Editor license found" with 0 entitlements; activation requires a human Unity-account `.alf`→`.ulf` exchange; no Windows host. The upgrade path is not exercisable (single shipped version; removal → reinstall is the qualified lifecycle). GAP-003 retains the distributed-kit view of these items. (Unity 6 / 6000.0 is now the qualified minimum host declaration; the `6000.0` floor itself remains untested, same license blocker.)
...
|---|---|---|---|
| 2026-09-26 (pass 17) | current source | GAP-002 closed by explicit availability recording (docs only; first-hand Linux editor run: batch mode exit 198 "No valid Unity Editor license found", 0 entitlements, changeset `a56f230f6470` tarball on x86_64 kernel 6.8.0-134) | OS/backend matrix row updated: macOS/Metal qualified; Windows/Linux recorded explicitly unavailable to this automation; 6000.0 minimum floor untested (same license blocker); upgrade path not exercisable (single shipped version). Installed-workflow §"Installed workflow and player build" open-items note replaced with the explicit unavailability record. GAP-002 register status set to closed on its retained pass-3/4/6 acceptance evidence. No host qualification, no pixels rendered. |
